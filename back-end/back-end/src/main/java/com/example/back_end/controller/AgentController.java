...
public class AgentController {
    ...
    public Map<String, Object> loginAgent(@RequestBody Map<String, String> request) {
        ...
        Optional<Agence> found = agenceRepository.findAll()
                ...
                .filter(a -> email.equalsIgnoreCase(a.getEmailSotadmin())
                        && matchesPassword(password, a.getPassword()))
                .findFirst();
    ...
    public Agence updateMyAgence(@PathVariable String id, @RequestBody Agence request) {
        ...
        if (request.getPassword() != null && !request.getPassword().isBlank()) {
            agence.setPassword(passwordEncoder.encode(request.getPassword()));
        }
    ...
    private boolean matchesPassword(String rawPassword, String storedPassword) {
        ...
        // Check if stored password is BCrypt encoded
        if (storedPassword.startsWith("$2a$") || storedPassword.startsWith("$2b$") || storedPassword.startsWith("$2y$")) {
            return passwordEncoder.matches(rawPassword, storedPassword);
        }
        // Legacy plaintext comparison (will be replaced when agent changes password)
    ...
    @GetMapping("/sinistres/{agenceId}")
    public List<Sinistre> getMyAgencyClaims(@PathVariable String agenceId) {
        Agence agence = agenceRepository.findById(agenceId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Agence non trouvée"));

        String nomAgence = agence.getNomAgence();
        List<String> numerosContrat = contratReferenceService.findAll().stream()
                .filter(c -> nomAgence != null && nomAgence.equals(c.getNomAgence()))
                .map(ContratReference::getNumeroContrat)
                .filter(s -> s != null && !s.isBlank())
                .map(String::trim)
                .distinct()
                .toList();

        if (numerosContrat.isEmpty()) {
            return List.of();
        }

        return sinistreService.findByNumeroContrats(numerosContrat);
    }

    @PatchMapping("/sinistres/{id}/statut")
    public Sinistre updateSinistreStatut(@PathVariable String id, @RequestBody UpdateStatutRequest request) {
        return sinistreService.updateStatut(id, request != null ? request.getStatut() : null);
    }

    @DeleteMapping("/sinistres/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteSinistre(@PathVariable String id) {
