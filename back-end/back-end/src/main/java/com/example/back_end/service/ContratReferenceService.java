...
public class ContratReferenceService {
    ...
    private void applyEditableFields(ContratReference target, ContratReference source) {
        ...
        if (source.getFichier() != null && source.getFichier().length > 0) {
            target.setFichier(source.getFichier());
        }

        if (source.getContenuContrat() != null) {
            target.setContenuContrat(source.getContenuContrat());
        }

        if (!StringUtils.hasText(target.getContenuContrat()) && target.getFichier() != null && target.getFichier().length > 0) {
            target.setContenuContrat(extractPlainTextFromContractBytes(target.getFichier()));
        }
    }

    private static String extractPlainTextFromContractBytes(byte[] bytes) {
        ...
        try (PDDocument pdf = PDDocument.load(bytes)) {
            ...
            // Not a valid PDF, try plain text fallback.
        }

        try {
            String plain = new String(bytes, StandardCharsets.UTF_8);
